...
async function processReservationReminders() {
    try {
        const reservationsNeedingReminder =
            await Reservation.findNeedingReminder();

        console.log(
            `[Jobs] Processing ${reservationsNeedingReminder.length} reminder(s)`
        );
        ...
        for (const reservation of reservationsNeedingReminder) {
            ...
            try {
                ...
                console.log(
                    `[Jobs] Reminder email sent to ${reservation.user.email} for locker ${reservation.locker.number}`
                );
                ...
                console.error(
                    `[Jobs] Failed to send reminder email:`,
                    emailErr.message
                );
            }
        }
...
async function processExpiredReservations() {
    try {
        ...
        console.log(
            `[Jobs] Processing ${expiredReservations.length} expired reservation(s)`
        );
        ...
        for (const reservation of expiredReservations) {
            ...
            await Locker.findByIdAndUpdate(reservation.locker._id, {
                status: "available",
            });
            ...
            try {
                ...
                console.log(
                    `[Jobs] Expiration email sent to ${reservation.user.email}`
                );
            } catch (emailErr) {
                console.error(
                    `[Jobs] Failed to send expiration email:`,
                    emailErr.message
                );
            }

            console.log(
                `[Jobs] Reservation ${reservation._id} expired - Locker ${reservation.locker.number} now available`
            );
        }
    } catch (error) {
