...
export const createLocker = async (req, res) => {
    ...
    if (!number || !size || price === undefined) {
        return res
            .status(400)
            .json({ error: "Number, size and price are required." });
    }

    if (!["small", "medium", "large"].includes(size)) {
        return res
            .status(400)
            .json({ error: "Size must be small, medium or large." });
    }
    ...
    try {
        ...
        if (existingLocker) {
            return res
                .status(409)
                .json({ error: "Locker number already exists." });
        }
...
export const updateLocker = async (req, res) => {
    ...
    if (status && !["available", "reserved", "maintenance"].includes(status)) {
        return res
            .status(400)
            .json({
                error: "Status must be available, reserved or maintenance.",
            });
    }
