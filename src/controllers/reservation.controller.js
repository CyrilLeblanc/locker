...
export const createReservation = async (req, res) => {
    ...
    if (!lockerId || !hours) {
        return res
            .status(400)
            .json({ error: "Locker ID and hours are required." });
    }
    ...
    if (isNaN(hoursNum) || hoursNum < 1 || hoursNum > 72) {
        return res
            .status(400)
            .json({ error: "Hours must be between 1 and 72." });
    }
    ...
    try {
        ...
        if (overlapping) {
            return res
                .status(409)
                .json({ error: "Locker is already reserved for this period." });
        }
...
export const cancelReservation = async (req, res) => {
    ...
    try {
        ...
        if (reservation.status !== "active") {
            return res
                .status(400)
                .json({ error: "Only active reservations can be cancelled." });
        }
        ...
        try {
            await sendReservationReturnedEmail(
                req.user.email,
                reservation,
                locker
            );
            ...
            console.error(
                "Failed to send reservation return email:",
                emailErr.message
            );
        }
