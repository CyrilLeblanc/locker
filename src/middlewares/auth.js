...
export const authenticate = async (req, res, next) => {
  ...
  if (error) {
    return res.status(401).json({ error: error === 'User not found' ? 'Invalid token. User not found.' : 'Invalid token.' });
  }
...
export const canAccessResource = (user, resourceUserId) => {
  return user._id.toString() === resourceUserId.toString() || user.role === 'admin';
};
